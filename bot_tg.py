...
bot_URL = 'http://robotbinance-3.herokuapp.com'
...
async def command_start(message : types.Message):
	try:
		await bot.send_message(message.from_user.id, '🤖Привет, {0.first_name} ,меня зовут Robot_Binance ,я представляю вам бота для торговли криптовалютой, чем я могу помочь?👨🏼‍💻 \nhttps://t.me/Robot_Binance_Chat - Здесь вы можете задать любые вопросы,а так же узнать результаты наших клиентов😊  Для получения информации можете воспользоваться подсказками ниже⬇️'.format(message.from_user), reply_markup= kb_client)
...
async def command_admin(message : types.Message):
	await message.reply('✅Напишите ему:\nhttp://t.me/@rodlonov1, если он не отвечает, можете обратиться к ним: \nhttp://t.me/@uropb1221👨🏼‍💻  \nhttp://t.me/@timoX01👨🏼‍💻')

@dp.message_handler(commands=['Прайс🗒'])
async def command_admin(message : types.Message):
	await message.reply('📊Прайс лист по установке торгового бота: Стоимость первичной настройки: 149$  Дополнительная настройка и постоянное обслуживание 39$💯')

@dp.message_handler(commands=['Стратегия🔸'])	
async def command_admin(message : types.Message):
	await message.reply('Стратегия бота заключается в сеточной торговле. Бот выставляет защитные ордера на усреднение. Условно, бот входит в сделку на 0,5-2% от депозита, и выставляет ордера. Если цена падает, бот усредняет позицию, как только происходит откат бот фиксирует прибыль. Сделок за день может быть не ограниченное количество, в зависимости от волатильности рынка. Торгует бот только в лонг и закрывает только прибыль. Чтобы убедиться в этом можете перейти к нам в канал, там мы транслируем каждый день результаты публичного аккаунта \nhttps://t.me/Robot_Binance_Chat ')
